...
def main():
    ...
    def run(item):
        ...
        status.write_text(json.dumps({"batch":n,"requested":len(rs),"status":"COMPLETED" if p.returncode==0 else "FAILED","returncode":p.returncode,"stdout":p.stdout[-4000:],"stderr":p.stderr[-4000:]},ensure_ascii=False,indent=2),encoding="utf-8")
